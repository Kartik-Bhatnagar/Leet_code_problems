...
#Date 12/08/2024
#instead of finding the index we will pass the element postion in bs function
        ...
        def bs(start,end):
            ...
            if mid!=start:
                ...
                if v < self.nums[mid]:
                    ...
                    # last_indx=len(self.nums) -1 - self.nums[::-1].index(self.nums[mid]) 
                    # indx= self.nums.index(lis[mid])
                    self.nums.insert(mid+1,v)                
        bs(0,len(self.nums))
